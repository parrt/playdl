...
# From http://www.paulgraham.com/talk.html
# text = get_text("notebooks/data/pg-essay.txt")
text = get_text("notebooks/data/shakespear.txt")
text = text.lower()
chars = list(text)
vocab = sorted(set(chars))
index = {w:i for i,w in enumerate(vocab)}
tokens = [index[w] for w in chars]
print(f"{len(vocab)} chars in vocab")
...
with torch.no_grad():
    h0 = torch.zeros(nhidden, 1, dtype=torch.float64) # reuse as we generate chars
    h = h0
    wi = wtoi[text[0]]
    print(text[0], end='')
    for i in range(150):
        _, o = model(h, torch.tensor([wi]))
        p = F.softmax(o, dim=1).numpy()[0]
